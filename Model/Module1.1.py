...
for index, row in df_met_buurschade.iterrows():
    ...
    for item in row['extra_info']:
        ...
        if row['hm_van'] == item[4]:  
            df_met_buurschade.at[index, 'middenachter_buur'] = [item[5], item[6]]

        #als de hm van match met de hm_tot van het huidige (middelste) vakje ligt het wegdeel ervoor
        elif row['hm_tot'] == item[3]:
            df_met_buurschade.at[index, 'middenvoor_buur'] = [item[5], item[6]]

        #links rechts bepalen
        elif row['strookvolg'] > item[2] and row['strooksoor'] == item[1]:
            df_met_buurschade.at[index, 'links_buur'] = [item[5], item[6]]
        #links rechts bepalen
        elif row['strookvolg'] < item[2] and row['strooksoor'] == item[1]:
            df_met_buurschade.at[index, 'rechts_buur'] = [item[5], item[6]]
